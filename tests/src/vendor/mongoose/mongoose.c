...
static void mg_ws_handshake(struct mg_connection *nc,
  ...
  char b64_sha[30];
  msgs[0] = (const uint8_t *) key->p;
  msgs[1] = (const uint8_t *) magic;
  msg_lens[0] = key->len;
  msg_lens[1] = 36;
...
static mg_event_handler_t mg_http_get_endpoint_handler(
  ...
  ep = pd->endpoints;
  while (ep != NULL) {
    struct mg_str name_s;
    name_s.p = ep->name;
    name_s.len = ep->name_len;
...
int mg_match_prefix_n(const struct mg_str pattern, const struct mg_str str) {
  ...
  if ((or_str = (const char *) memchr(pattern.p, '|', pattern.len)) != NULL) {
    struct mg_str pstr;
    pstr.p = pattern.p;
    pstr.len = (size_t)(or_str - pattern.p);
  ...
  for (; i < pattern.len; i++, j++) {
    if (pattern.p[i] == '?' && j != str.len) {
      ...
      do {
        ...
        struct mg_str sstr;
        pstr.p = pattern.p + i;
        pstr.len = pattern.len - i;
        sstr.p = str.p + j + len;
        sstr.len = str.len - j - len;
